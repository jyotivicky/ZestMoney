...
import testBase.globalVar;
	...
	/*
	 * to select a date from the date list based on the integer entered from date
	 * list
	 */
	public static void SelectNewDate(WebDriver driver, List<WebElement> DateList, int SelectDate) {
		List<WebElement> Dates = DateList;
		for (int i = 0; i < Dates.size(); i++) {
			String GetDate = Dates.get(i).getText();
			int result = Integer.valueOf(GetDate);
			if (result == SelectDate) {
				DateList.get(i).click();
			}
		}
	}
	...
	/*
	 * to return a value after randomly selected a value from a select box also this
	 * is to handle the exception to select the item using try and catch
	 */
	public static WebElement GetIndexSelectedValue(WebElement element) throws InterruptedException {
		WebElement SelectList = element;
		Select objSel = new Select(SelectList);
		List<WebElement> weblist = objSel.getOptions();
		Random num = new Random();
		int iSelect = num.nextInt(weblist.size());
		wait_20();
		...
		while (attempts3 < 10) {
			try {
				...
				wait_20();
				SelectedVisibleValue = objSel.getFirstSelectedOption();
				wait_30();
				result3 = true;
				break;
